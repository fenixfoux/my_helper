...
def modify_task_recursive(new_task: dict, tasks: list, parent_task_id, operation_type):
    for task in tasks:
        if task['task_id'] == parent_task_id:
            ...
            print(task)
            if operation_type == all_vars.all_operation_types['add']:
                task['task_subtasks'].append(new_task)
            elif operation_type == all_vars.all_operation_types['update']:
            ...
            elif operation_type == all_vars.all_operation_types['delete']:
                tasks.remove(task)
            else:
...
def task_crud(main_list_tasks, task_id, task_name, task_description, parent_task_id=None, operation_type=''):
...
def save_tasks_to_json(tasks):
    # with open("all_tasks.json", "w", encoding='utf-8') as json_file:
